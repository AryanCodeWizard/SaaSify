...
class GoDaddyService {
  constructor() {
    ...
    if (!this.apiKey || !this.apiSecret) {
      ...
      logger.info(`✅ GoDaddy service initialized (${this.isProduction ? 'Production' : 'OTE Test'} environment)`);
    }
  ...
  async checkAvailability(domain) {
    try {
      ...
      return {
        ...
        price: response.data.price ? response.data.price / 1000000 : null, // Convert from micros to dollars
        currency: response.data.currency || 'USD',
        period: response.data.period || 1,
        definitive: response.data.definitive || false,
      };
    } catch (error) {
      logger.error(`Domain availability check failed for ${domain}:`, {
        message: error.message,
        status: error.response?.status,
        data: error.response?.data,
      });
  ...
  async checkBulkAvailability(domains) {
    try {
      ...
      return response.data.domains.map((domain) => ({
        ...
        price: domain.price ? domain.price / 1000000 : null, // Convert from micros to dollars
        currency: domain.currency || 'USD',
        period: domain.period || 1,
        definitive: domain.definitive || false,
      }));
    } catch (error) {
      logger.error('Bulk domain availability check failed:', {
        message: error.message,
        status: error.response?.status,
      });
      throw new Error('Failed to check domain availability');
  ...
  async getDomainSuggestions(query, options = {}) {
    try {
      ...
      logger.error('Domain suggestions failed:', {
        message: error.message,
        status: error.response?.status,
      });
  ...
  async getDomainPricing(tld) {
    try {
      ...
      return {
        ...
        prices: {
          registration: {
            1: response.data.prices?.registration?.price ? response.data.prices.registration.price / 1000000 : null, // Convert from micros to dollars
            currency: response.data.prices?.registration?.currency || 'USD',
          },
          renewal: {
            1: response.data.prices?.renewal?.price ? response.data.prices.renewal.price / 1000000 : null, // Convert from micros to dollars
            currency: response.data.prices?.renewal?.currency || 'USD',
          },
          transfer: {
            1: response.data.prices?.transfer?.price ? response.data.prices.transfer.price / 1000000 : null, // Convert from micros to dollars
            currency: response.data.prices?.transfer?.currency || 'USD',
          },
        },
      };
    } catch (error) {
      logger.error(`Domain pricing failed for TLD ${tld}:`, {
        message: error.message,
        status: error.response?.status,
      });
