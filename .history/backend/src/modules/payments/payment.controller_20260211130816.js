...
export const createWalletRazorpayOrder = async (req, res) => {
  try {
    ...
    // Generate short receipt for Razorpay (max 40 chars)
    const receipt = `wlt_${client._id.toString().slice(-8)}_${Date.now()}`.slice(0, 40);
    ...
    logger.error('Create wallet Razorpay order failed:', error);
    return errorResponse(res, 'Failed to create wallet order', 500);
  }
};
