  ...
  try {
    ...
    for (const item of domainItems) {
      ...
      const domain = await Domain.create([{
        ...
        registeredAt: null,
        expiresAt: new Date(Date.now() + (item.billingCycle || 1) * 365 * 24 * 60 * 60 * 1000),
        autoRenew: item.configOptions?.autoRenew !== false,
        privacyProtection: item.configOptions?.privacy !== false,
        status: 'pending',
        registrationPeriod: item.billingCycle || 1,
