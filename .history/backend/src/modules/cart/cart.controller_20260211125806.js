...
export const checkout = async (req, res) => {
  try {
    ...
    if (paymentMethod === 'razorpay') {
      try {
        // Create Razorpay order (razorpayService will handle currency conversion)
        const razorpayOrder = await razorpayService.createOrder({
          amount: cart.total, // Service will convert to smallest unit
        ...
        logger.info(`Razorpay order created: ${razorpayOrder.id} for order ${order[0].orderNumber}`);
...
export const verifyPayment = async (req, res) => {
  try {
    ...
    // Get client for wallet operations
    const client = await Client.findOne({ userId });
    const user = await User.findById(userId);
    ...
    for (const item of domainItems) {
      ...
      const domain = await Domain.create([{
        ...
        tld: item.metadata?.tld,
        registeredAt: null,
        expiresAt: new Date(Date.now() + (item.metadata?.period || 1) * 365 * 24 * 60 * 60 * 1000),
        autoRenew: item.metadata?.autoRenew !== false,
        privacyProtection: item.metadata?.privacy !== false,
        status: 'pending',
        registrationPeriod: item.metadata?.period || 1,
        registrationPrice: item.price,
        renewalPrice: item.price,
        nameservers: item.metadata?.nameServers || [],
      }]);
      ...
      try {
        await domainRegistrationQueue.add(
          'register-domain',
          {
            ...
            domainData: {
              domain: domain[0].domainName,
              period: domain[0].registrationPeriod,
              renewAuto: domain[0].autoRenew,
              privacy: domain[0].privacyProtection,
              nameServers: domain[0].nameservers,
              contactRegistrant: {
                firstName: order.billingDetails?.firstName || client.firstName,
                lastName: order.billingDetails?.lastName || client.lastName,
                email: user.email,
                phone: order.billingDetails?.phone || client.phone || '+1.0000000000',
                organization: client.companyName || '',
                address: {
                  street: order.billingDetails?.address?.street || '123 Main St',
                  city: order.billingDetails?.address?.city || 'New York',
                  state: order.billingDetails?.address?.state || 'NY',
                  country: order.billingDetails?.address?.country || 'US',
                  zipCode: order.billingDetails?.address?.zipCode || '10001',
                },
              },
              consent: {
                agreementKeys: ['DNRA', 'DNPA'],
