...
export const getWalletTransactions = async (req, res) => {
  try {
    ...
    // Query for wallet-related transactions:
    // 1. Payments (type: payment) - wallet top-ups from payment gateways
    // 2. Wallet payments (gateway: wallet) - payments made from wallet balance
    const query = {
      clientId: client._id,
      $or: [
        { type: 'payment' },
    ...
    if (type) {
      // If specific type requested, override the $or query
      query.$or = undefined;
      query.type = type;
    }
