...
export default function Cart() {
  ...
  const updateQuantity = async (domainName, quantity) => {
    if (quantity < 1) return;
    
    try {
      setUpdatingItems(prev => ({ ...prev, [domainName]: true }));
      await cartService.updateCartItem(domainName, quantity);
      await loadCart();
    } catch (err) {
      setError(err.message || 'Failed to update quantity');
    } finally {
      setUpdatingItems(prev => ({ ...prev, [domainName]: false }));
    }
  };

  const removeItem = async (domainName) => {
    try {
      setUpdatingItems(prev => ({ ...prev, [domainName]: true }));
      await cartService.removeFromCart(domainName);
      await loadCart();
    } catch (err) {
